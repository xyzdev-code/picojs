...
__export(index_exports, {
  ...
  VALID_HEAD_ELEMENTS: () => VALID_HEAD_ELEMENTS,
  analyzeHead: () => analyzeHead,
  analyzeHeadWithOrdering: () => analyzeHeadWithOrdering,
  checkOrdering: () => checkOrdering,
  getCustomValidations: () => getCustomValidations,
  getHeadWeights: () => getHeadWeights,
  getValidationWarnings: () => getValidationWarnings,
  getWeight: () => getWeight,
  getWeightCategory: () => getWeightCategory,
  hasValidationWarning: () => hasValidationWarning,
  isAsyncScript: () => isAsyncScript,
  isDeferScript: () => isDeferScript,
  isImportStyles: () => isImportStyles,
  isMeta: () => isMeta,
  isMetaCSP: () => isMetaCSP,
  isOriginTrial: () => isOriginTrial,
  isPreconnect: () => isPreconnect,
  isPrefetchPrerender: () => isPrefetchPrerender,
  isPreload: () => isPreload,
  isSyncScript: () => isSyncScript,
  isSyncStyles: () => isSyncStyles,
  isTitle: () => isTitle,
...
function isMeta(element, adapter) {
...
function isTitle(element, adapter) {
  return adapter.getTagName(element) === "title";
}
function isPreconnect(element, adapter) {
...
function isAsyncScript(element, adapter) {
...
function isSyncScript(element, adapter) {
  ...
  if (adapter.hasAttribute(element, "src")) {
    const type2 = adapter.getAttribute(element, "type");
    if (type2 && type2.toLowerCase() === "module") {
      return false;
    }
  }
...
function isPreload(element, adapter) {
  if (adapter.getTagName(element) !== "link") {
    return false;
  }
  const rel = adapter.getAttribute(element, "rel");
...
function isDeferScript(element, adapter) {
  if (adapter.getTagName(element) !== "script") {
    return false;
  }
  if (!adapter.hasAttribute(element, "src")) {
...
function isPrefetchPrerender(element, adapter) {
  if (adapter.getTagName(element) !== "link") {
    return false;
  }
  const rel = adapter.getAttribute(element, "rel");
...
function isOriginTrial(element, adapter) {
...
function isMetaCSP(element, adapter) {
  if (adapter.getTagName(element) !== "meta") {
    return false;
  }
  const httpEquiv = adapter.getAttribute(element, "http-equiv");
...
function getHeadWeights(head, adapter) {
...
var VALID_HEAD_ELEMENTS = /* @__PURE__ */ new Set([
  "base",
  "link",
  "meta",
  "noscript",
...
function isValidElement(element, adapter) {
  const tagName = adapter.getTagName(element);
  if (!tagName || tagName === "") {
    return true;
  }
...
function hasValidationWarning(element, adapter) {
...
function getValidationWarnings(head, adapter) {
...
function getCustomValidations(element, adapter, parentElement = null) {
  const results = [];
...
function decodeOriginTrialToken(token) {
  ...
  payload.expiry = new Date(payload.expiry * 1e3);
  return payload;
}
...
function validateMetaViewport(element, adapter) {
  ...
  if ("width" in directives) {
    ...
    if (Number(width) < 1 || Number(width) > 1e4) {
  ...
  if ("height" in directives) {
    ...
    if (Number(height) < 1 || Number(height) > 1e4) {
  ...
  const validDirectives = /* @__PURE__ */ new Set([
    "width",
    "height",
    "initial-scale",
    "minimum-scale",
...
function analyzeHead(headNode, adapter, options = {}) {
  ...
  const weights = getHeadWeights(headNode, adapter);
  const validationWarnings = includeValidation ? getValidationWarnings(headNode, adapter) : [];
  const customValidations = includeCustomValidations ? getElementValidations(headNode, adapter) : [];
  return {
    weights,
    validationWarnings,
    customValidations,
...
function getWeightCategory(weight) {
  for (const [category, value] of Object.entries(ElementWeights)) {
    if (value === weight) {
      return category;
    }
  }
...
function checkOrdering(weights) {
  const violations = [];
...
function analyzeHeadWithOrdering(headNode, adapter, options = {}) {
...
var BrowserAdapter = class extends AdapterInterface {
  /**
   * Check if node is an Element (not text, comment, etc.)
   * @override
   * @param {any} node - The node to check
